...
submitBtn.addEventListener('click', (e) => {
    ...
    let seed_quantity = values[1].value;
    let equipment_id = values[2].value;
    let farmland_address = values[3].value;
    let person_pesel = values[4].value;

    return fetch('/update/sowing', {
      method: 'PUT',
      headers: { 'Content-type': 'application/json' },
      body: JSON.stringify({ recent_activity, seed_quantity, equipment_id, farmland_address, person_pesel })
    });
});